...
export default async function PreviewPage({ params }: { params: { token: string } }) {
  ...
  // Media rides on the preview token: this page has no session to read the normal
  // site-scoped media route with.
  const html = renderDocumentToHtml(data.article.document, {
    mediaUrl: (mediaId) => `${API_BASE}/v1/preview/${encodeURIComponent(params.token)}/media/${encodeURIComponent(mediaId)}/file`,
  });
  ...
  return (
    <main style={{ maxWidth: 720, margin: "0 auto", padding: 24, fontFamily: "Georgia, serif", lineHeight: 1.7 }}>
      ...
      <style>{`
        ...
        article figure img { max-width: 100%; height: auto; display: block; border-radius: 4px; }
        article figure.gallery { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 8px; }
        article figure.gallery img { height: 100%; object-fit: cover; }
